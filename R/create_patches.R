...
#'
#'@description This function takes a feature and splits it into multiple
#'  "patches" of the feature. This is ideal for features that you want to
#'  protect x% of, but want to make sure that entire patches (as opposed to
#'  pieces of patches) are protected to meet that target.
#'
#'@param feature a raster or sf object with the feature of interest present
#'  (value = 1) or absent (value = NA)
#'@param spatial_grid a raster or sf template with the desired resolution and
#'  coordinate reference system generated; values in areas of interest are 1,
#'  while all other values are NA (only required if feature is a sf object)
#'
#'@return A raster or sf object with independent layers (raster)/columns (sf)
