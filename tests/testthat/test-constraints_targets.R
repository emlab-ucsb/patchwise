test_that("create constraints - raster", {
  expect_s3_class(
    ...
    patches_df_rast <- create_patch_df(spatial_grid = planning_rast, features = features_rast, patches = patches_rast, costs = cost_rast)

    # Create boundary matrix
    bnd_mat_rast <- create_boundary_matrix(spatial_grid = planning_rast,
    ...
    # Create constraints
    constraints_targets(feature_targets = targets_rast, patch_df = patches_df_rast)
    },
    class = "tbl_df")
})
...
test_that("create constraints - sf", {
  expect_s3_class(
    ...
    # Create seamount patches - seamount areas that touch are considered the same patch
    patches_sf <- create_patches(seamounts_sf, spatial_grid = planning_sf)
    ...
    # Create patches dataframe - this creates several constraints so that entire seamount units are protected together
    patches_df_square <- create_patch_df(spatial_grid = planning_sf, features = features_sf, patches = patches_sf, costs = cost_sf)

    # Create boundary matrix
    bnd_mat_square <- create_boundary_matrix(spatial_grid = planning_sf,
                                             patches = patches_sf,
    ...
    # Create constraints
    constraints_targets(feature_targets = targets_sf, patch_df = patches_df_square)
    },
    class = "tbl_df")
})
